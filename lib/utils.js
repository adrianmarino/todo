...
function ResponseHandler(converter) {
    ...
    var json = function(response, result) {
        response.json(converter.toApi(result));
    }

    this.toJsonCallback = function(response) {
        return function(error, result) {
            if (error)
                util.internalServerError(response, error);
            else
                isEmpty(result) ? util.notFound(response) : json(response, result)
        };
    };

    this.errorHandlerCallback = function(response) {
        return function(error, result) {
